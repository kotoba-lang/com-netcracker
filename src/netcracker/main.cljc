(ns netcracker.main
  "Kotodama WASM entrypoint for the Netcracker clean-room actor (L4) — Clojure port.
  ...
  facts (`netcracker.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "netcracker")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"netcracker.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "netcracker-compat" :tier tier :entities entities} 200])
...
(defn start! [] :netcracker-compat/ready)
